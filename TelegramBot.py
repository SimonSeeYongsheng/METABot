...
async def feedback_with_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id : str, object_id: str, 
                                 feedback_message : str, category: str):

    metadata = f"{{sentiment}}|{category}|{object_id}"
        ...
        case original_question if original_question == guidance_query_message or original_question == teach_query_message:
            ...
            if result:
                ...
                if context.user_data["category"] == "teach":
                    best_object_id, best_response, best_like, best_dislike = chat_db.get_saved_response_teach(result)

                else:
                    best_object_id, best_response, best_like, best_dislike = chat_db.get_saved_response_guide(result)

                await reply_to_query(update=update, context=context, user_id=user_id, response=best_response)
                await feedback_with_callback(update=update, context=context, user_id=user_id, object_id=best_object_id, 
                                 feedback_message=second_feedback_message.format(likes=best_like, dislikes=best_dislike), category=context.user_data["category"])
            else:
